...
void PixelShader( const Pixel& p) {
	...
	if( p.zinv >= depthBuffer[x][y]) {
		...
		//vec3 color = p.illumination;//ComputeLight(p.pos3D, lightPos, currentReflectance, currentNormal);
...
void Interpolate( Pixel a, Pixel b, vector<Pixel>& result) {
	...
	for (int i = 0; i<numSamples; ++i) {
		...
		nextSample.pos3D = (a.pos3D + i*deltaPos) / nextSample.zinv;
	}
}
